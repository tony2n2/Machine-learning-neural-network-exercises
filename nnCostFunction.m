...
a1 = [biasNodes, X];
z2 = a1*Theta1';
a2 = [biasNodes, sigmoid(z2)];
z3 = a2*Theta2';
a3 = sigmoid(z3);
y_bin = full(sparse(1:numel(y), y, 1, numel(y), num_labels));
J = sum(sum((-y_bin .* log(a3)) - ((1 - y_bin) .* (log(1-a3))),2));
J /= m;

J += lambda/(2*m)*(sum(sum(Theta1(1:size(Theta1,1),2:size(Theta1,2)).^2)) + ...
     sum(sum(Theta2(1:size(Theta2,1),2:size(Theta2,2)).^2)));

% Backpropagation of t_th training example
%for t = 1:m,
